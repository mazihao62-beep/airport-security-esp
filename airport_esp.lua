--[[
    ...
    v12.6 分类器全面简化:
    只保留属性 + 名字 + 路径 三层检查
    + Debug Mode: 头顶显示NPC原始名字+所有属性
    + 无法判断 -> 跳过（不再默认为坏人）
    粒子颜色改用当前主题色（不再是硬编码蓝色）
]]
...
local function mt()if WN then pcall(function()VIM:SendKeyEvent(true,Enum.KeyCode.RightShift,false,game);task.wait(0.05);VIM:SendKeyEvent(false,Enum.KeyCode.RightShift,false,game)end)end end
...
-- v12.6 粒子颜色跟随主题: getParticleColor() -> gtc(S.CurrentTheme)
local function cp()
    ...
    pcall(function()
        ...
        local col=gtc(S.CurrentTheme);local w=s.X;local h=s.Y -- v12.6: 粒子颜色取自当前主题
...
local function classify(c,h)
    ...
    if h then
        for _,an in ipairs(AN)do local v=nil;pcall(function()v=h:GetAttribute(an)end);if v then local vs=tostring(v):lower();for _,g in ipairs(AG)do if vs:find(g,1,true)then dp("  A+ "..an.."="..tostring(v));return"Good"end end;for _,b in ipairs(AB)do if vs:find(b,1,true)then dp("  A- "..an.."="..tostring(v));return"Bad"end end end end
        pcall(function()for _,a in ipairs(h:GetAttributes())do dp("  ATTR "..a.."="..tostring(h:GetAttribute(a)))end end)
    end
    local nl=nm:lower()
...
local function me(c,nt)
    ...
    if S.DebugMode then
        local at=""
        pcall(function()for _,a in ipairs(c:GetAttributes())do at=at..a.."="..tostring(c:GetAttribute(a)).." "end end)
        local h2=c:FindFirstChildOfClass("Humanoid")
        if h2 then pcall(function()for _,a in ipairs(h2:GetAttributes())do at=at..a.."="..tostring(h2:GetAttribute(a)).." "end end)end
        inf.Text=c.Name..(at~=""and(" | "..at)or"")
    end
    EO[c]={HL=hl,BB=bb,LB=lb,Inf=inf,HD=hd,RT=rt,NT=nt};ST.S=ST.S+1;TR[c]=nt
end

local function re(c)if EO[c]then local o=EO[c];pcall(function()o.HL:Destroy()end);pcall(function()o.BB:Destroy()end);EO[c]=nil;ST.S=math.max(0,ST.S-1);TR[c]=nil end end
local function ce()for c,_ in pairs(EO)do if not c or not c.Parent then re(c)end end end
local function ue()for _,o in pairs(EO)do local s=S.Enabled and(not S.BadOnly or o.NT=="Bad");if o.HL then o.HL.Enabled=s end;if o.BB then o.BB.Enabled=s end end end

local function sc()
    if IS then return end;IS=true;dp("=====SCAN=====")
    pcall(function()
        for _,o in ipairs(WS:GetDescendants())do
            local h,c=nil,nil
            if o:IsA("Humanoid")then h=o;c=o.Parent end
            if c and h and not TR[c]and not isP(c)then
                if c:FindFirstChild("Head")or c:FindFirstChild("HumanoidRootPart")then
                    local nt=classify(c,h);if nt then
...
if ok and rv then
    ...
    WI:Popup({Title="机场安全透视 v12.6",Icon="solar:info-square-bold",Content="👁 透视高亮 + 头顶标签\n🔍 简化分类器: 属性->名字->路径\n📋 Debug模式: 头顶显示原始信息\n⚠️ 无法判断的NPC自动跳过\n\n⚠️ 所有功能默认关闭",
        Buttons={{Title="取消",Callback=function()end,Variant="Tertiary"},{Title="确认加载",Icon="solar:arrow-right-bold",Callback=function()PP=true;pcall(function()WI:Notify({Title="✅ 已加载",Content="按 RightShift 打开菜单\n调试日志在【信息统计】Tab",Duration=4,Icon="solar:bell-bold"})end);task.spawn(function()cw();task.wait(1);sc()end)end,Variant="Primary"}}})
    task.spawn(function()
        while not PP do task.wait(0.5)end;task.wait(0.5);bu()
        task.spawn(function()while true do pcall(function()ce();if S.Enabled then sc()end end);task.wait(5)end end)
        ...
        UIS.InputBegan:Connect(function(i,g)if g then return end;if i.UserInputType~=Enum.UserInputType.Keyboard then return end;local kn=i.KeyCode.Name
            if KB.ESP and KB.ESP~=""and kn==KB.ESP then S.Enabled=not S.Enabled;pcall(function()if CT.ESP then CT.ESP:Set(S.Enabled)end end);ue();if S.Enabled then task.spawn(sc)end end
            if KB.BadOnly and KB.BadOnly~=""and kn==KB.BadOnly then S.BadOnly=not S.BadOnly;pcall(function()if CT.BO then CT.BO:Set(S.BadOnly)end end);ue()end end)
    end)
    ...
    function cw()
        ...
        CT.ESP=mt:Toggle({Flag="ESPToggle",Title="透视开关",Value=false,Callback=function(v)S.Enabled=v;ue();if v then task.spawn(sc)end end})
        CT.BO=mt:Toggle({Flag="BadOnlyToggle",Title="仅显示坏人",Value=false,Callback=function(v)S.BadOnly=v;ue()end})
        ...
        CT.HT=mt:Toggle({Flag="HealthToggle",Title="显示血量",Value=false,Callback=function(v)S.ShowHealth=v end})
        mt:Divider()
        ...
        mt:Divider();mt:Paragraph({Title="🔬 诊断模式",Desc="在头顶显示NPC原始名字+所有属性"})
        CT.DM=mt:Toggle({Flag="DebugToggle",Title="Debug 模式",Value=false,Desc="头顶显示NPC原始信息（用于判断分类）",Callback=function(v)S.DebugMode=v end})

        local ft=WN:Tab({Title="功能设置",Icon="solar:settings-bold"})
        ft:Paragraph({Title="🔑 快捷键"})
        CT.EK=ft:Keybind({Flag="ESPKeybind",Title="透视开关",Value="",Callback=function(k)KB.ESP=k end})
        CT.BK=ft:Keybind({Flag="BadKeybind",Title="仅坏人模式",Value="",Callback=function(k)KB.BadOnly=k end})

        local ut=WN:Tab({Title="UI设置",Icon="solar:monitor-bold"})
        ut:Paragraph({Title="⚙️ 界面"})
        CT.WK=ut:Keybind({Flag="WinKeybind",Title="窗口开关",Value="RightShift",Callback=function(k)KB.Window=k;if WN then pcall(function()WN:SetToggleKey(Enum.KeyCode[k])end)end end})
        CT.FB=ut:Toggle({Flag="FloatBtn",Title="悬浮按钮",Value=IM,Callback=function(v)if FB then FB.Enabled=v end end})
        ut:Divider();ut:Paragraph({Title="🌀 背景"})
        CT.PT=ut:Toggle({Flag="PartToggle",Title="粒子背景",Value=true,Callback=function(v)S.Particles=v;if v then cp()else dp2()end end})
        ...
        ct:Paragraph({Title="💾 配置管理"})
        local cni=ct:Input({Flag="CfgName",Title="配置名称",Value="default",Icon="solar:file-text-bold",Callback=function(v)CF=v end});ct:Space()
        ...
        ct:Button({Title="💾 保存",Icon="solar:check-circle-bold",Justify="Center",Color=Color3.fromHex("#305dff"),Callback=function()if not CM then return end;pcall(function()local c=CM:Config(CF);if c and c:Save()then WI:Notify({Title="✅ 已保存",Content="配置 '"..CF.."'",Duration=3,Icon="solar:check-circle-bold"});ACD:Refresh(CM:AllConfigs())end end)end})
        ct:Space()
        ct:Button({Title="📂 加载",Icon="solar:refresh-circle-bold",Justify="Center",Color=Color3.fromHex("#10C550"),Callback=function()if not CM then return end;pcall(function()local c=CM:CreateConfig(CF,false);if c and c:Load()then WI:Notify({Title="✅ 已加载",Content="配置 '"..CF.."'",Duration=3,Icon="solar:refresh-circle-bold"})end end)end})
        ct:Space()
        ...
        at:Paragraph({Title="机场安全透视 v12.6",Desc="简化分类器 + Debug模式"})
        at:Divider();at:Paragraph({Title="👤 作者",Desc="b站英吉利超入_"})
        at:Divider();at:Paragraph({Title="💡 使用",Desc=IM and"手机: 点击👁"or"PC: RightShift打开菜单"})
        at:Paragraph({Title="🔬 Debug模式",Desc="开启后NPC头顶显示原始名字+所有属性"})
        at:Paragraph({Title="🧹 清理",Desc="执行: _G.CleanupESP()"})

        task.spawn(function()
            task.wait(1);pcall(function()
                FB=tg(Instance.new("ScreenGui"));FB.Name="AirportESP_Btn";FB.Enabled=IM;FB.ResetOnSpawn=false;FB.Parent=CG
    ...
    print("[错误] WindUI加载失败")
    local msg=Instance.new("Message");msg.Text="⚠️WindUI加载失败";msg.Parent=WS;task.delay(5,function()msg:Destroy()end)
end
print("[v12.6] 完成")
