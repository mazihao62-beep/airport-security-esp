...
local function isPlayerChar(m)
    ...
    for _,pl in ipairs(P:GetPlayers())do
        ...
        if pl.Name==m.Name then return true end
    end
    return false
end
...
local function rFind(inst,name)
    local f=inst:FindFirstChild(name);if f then return f end
    for _,c in ipairs(inst:GetChildren())do
        if c:IsA("Configuration")or c:IsA("Folder")then local r=rFind(c,name);if r then return r end end end
    return nil
end
...
local function scanProps(m)
    ...
    if sv then
        local h=rFind(sv,"Hostile")
        if h and h:IsA("BoolValue")and h.Value then return true end
    end
    local rv=rFind(pr,"RandomVariables")
    if rv then
        local c=rFind(rv,"ContrabandReal")
        if c and c:IsA("BoolValue")and c.Value then return true end
        local f=rFind(rv,"FakePassport")
    ...
    if hum then
        for _,an in ipairs({"NPCType","Type","Faction","Team","Role"})do
            ...
            if v then
                ...
                for _,g in ipairs({"agent","good","friendly","ally","police","guard","civilian","security"})do
                    if vs:find(g,1,true)then return"Good"end end
                for _,b in ipairs({"enemy","bad","hostile","terrorist","criminal"})do
                    if vs:find(b,1,true)then return"Bad"end end
            end
        end
    end
    -- Properties检测
    local bad=scanProps(c)
    if bad~=nil then return bad and"Bad"or"Good"end
    -- 名字检测
    local nl=nm:lower()
    for _,kw in ipairs({"警察","保安","警卫","police","guard","agent","officer","prisoner","store","npcstore","市民","商人"})do
        if nl:find(kw:lower(),1,true)then return"Good"end end
    for _,kw in ipairs({"恐怖","匪","敌人","坏","犯罪","terrorist","enemy","hostile","criminal","smuggler"})do
        if nl:find(kw:lower(),1,true)then return"Bad"end end
    -- 路径检测
    local pl=fp:lower()
    for _,kw in ipairs({"agent","police","friendly","civilian","prisoner","store","jail"})do
        if pl:find(kw,1,true)then return"Good"end end
    for _,kw in ipairs({"enemy","terror","hostile","criminal","invader"})do
        if pl:find(kw,1,true)then return"Bad"end end
    return"Good"
end
...
local function classifyLuggage(lug)
    ...
    if pr then
        ...
        if cb and cb:IsA("BoolValue")then return cb.Value and"Dangerous"or"Safe"end
    end
    return"Suspicious"
end
...
local function getLuggagePP(lug)
    ...
    if not pp then
        for _,c in ipairs(lug:GetDescendants())do
            if c:IsA("BasePart")and(c.Name=="ColorFront"or c.Name=="ColorBack"or c.Name=="HumanoidRootPart")then pp=c;break end end
    end
    if not pp then
...
local function makeHL(obj,col)
    ...
    hl.FillColor=col;hl.FillTransparency=0.25;hl.OutlineColor=Color3.fromRGB(255,255,255);hl.OutlineTransparency=0
...
local function makeNPC_ESP(c,nt)
    ...
    if isPlayerChar(c)then return end
    ...
    local hl=makeHL(c,col)
    local head=c:FindFirstChild("Head")or hrp
    local bb=Instance.new("BillboardGui")
    bb.Adornee=head;bb.Size=UDim2.new(0,220,0,56)
    ...
    ob.Size=UDim2.new(1,4,1,4);ob.Position=UDim2.new(0,-2,0,-2)
    ...
    lb.Size=UDim2.new(1,-6,1,0);lb.Position=UDim2.new(0,3,0,0)
    ...
    H[c]={bb=bb,lb=lb,hrp=hrp,nt=nt,tag=tag,hl=hl}
    SC=SC+1
    if nt=="Good"then GC=GC+1 else BC=BC+1 end
end
...
local function makeLuggage_ESP(lug,lt)
    ...
    local pp=getLuggagePP(lug);if not pp then return end
    local key=pp
    if LG[key]then
        if LG[key].nt~=lt then
            LG[key].nt=lt
            LG[key].tag=lt=="Dangerous"and"💣 危险行李"or(lt=="Safe"and"🧳 安全行李"or"❓ 可疑行李")
            if LG[key].lb then
    ...
    local hl=makeHL(lug,col)
    local bb=Instance.new("BillboardGui")
    bb.Adornee=pp;bb.Size=UDim2.new(0,220,0,56)
    ...
    lb.BackgroundTransparency=1;lb.TextColor3=col;lb.Font=Enum.Font.SourceSansBold
    lb.TextScaled=true;lb.Text=tag;lb.BorderSizePixel=0;lb.Parent=bg
    LG[key]={bb=bb,lb=lb,pp=pp,nt=lt,tag=tag,hl=hl}
    LC=LC+1
    if lt=="Dangerous"then LDC=LDC+1 elseif lt=="Safe"then LSC=LSC+1 end
end
                        ...
                        for _,lug in ipairs(fw:GetChildren())do
                            if lug:IsA("Model")and lug.Name=="OpenableLuggage"then
                                ...
                                if pp and not seenPP[pp]then
                                    seenPP[pp]=true
                                    local lt=classifyLuggage(lug)
                                    makeLuggage_ESP(lug,lt)
                                end
                            end
                        end
                    end
    ...
    if LC==0 then
        for _,o in ipairs(W:GetDescendants())do
            if o:IsA("Model")and o.Name=="OpenableLuggage"and o.Parent and o.Parent:IsA("Folder")then
                ...
                if pp and not seenPP[pp]then
                    seenPP[pp]=true
                    local lt=classifyLuggage(o)
                    makeLuggage_ESP(o,lt)
                end
            end
        end
    end
...
local function refreshESP()
        ...
        if not c or not c.Parent then
            ...
            if o.lb then
                ...
                if S.ShowDist and LP.Character then
                    ...
                    if mp and o.hrp then txt=txt.." | "..math.floor((o.hrp.Position-mp.Position).Magnitude+0.5).."m"end
                end
                if S.ShowHP then
                    local h2=c:FindFirstChildOfClass("Humanoid")
                    if h2 then txt=txt.." | HP:"..math.floor(h2.Health+0.5).."/"..math.floor(h2.MaxHealth+0.5)end
                end
                o.lb.Text=txt
            end
        end
    ...
    for _,o in pairs(LG)do
        ...
        if o.hl then o.hl.Enabled=S.Luggage end
    end
end
    ...
    task.spawn(function()
        ...
        if mode=="Arrest"or mode=="Auto"then
            ...
            if hostile and mode=="Auto"then
                pcall(function()if ATE.ST then ATE.ST:Set("[反击] "..npc.Name.." 反抗")end end)
            end
        end
        if mode=="Kill"or(mode=="Auto"and scanProps(npc))then
            if LP and LP.Character then
                for _,t in ipairs(LP.Character:GetChildren())do
                    if t:IsA("Tool")and(t.Name=="MP7A1"or t.Name=="M1911")then
...
local function mkParts()
    ...
    sg.Name="ESP_Particles";sg.ResetOnSpawn=false;sg.DisplayOrder=999999;sg.IgnoreGuiInset=true;sg.Parent=C
...
local function killParts()
    PR=false
    if PC then
        pcall(function()local p=PC.Parent;if p then p:Destroy()end end)
        PC=nil
    end
    PS={}
end
...
local function makeWindow()
    local ok2,w=pcall(function()
        return WI:CreateWindow({
            ...
            OnClose=function()
                S.Enabled=false;S.Luggage=false;S.AutoWork=false
                if CT.ESP then CT.ESP:Set(false)end
                if CT.LT then CT.LT:Set(false)end
                if CT.AW then CT.AW:Set(false)end
                refreshESP();killParts()
            end,
            OnOpen=function()
                if S.Particles then
                    task.spawn(function()task.wait(0.5);mkParts()end)
                end
            end,
        })
    end)
    ...
    mt:Divider()
    CT.LT=mt:Toggle({Flag="Luggage",Title="🧳 行李箱检测",Value=false,Callback=function(v)
        S.Luggage=v
        if v then task.spawn(rescanLuggage)
        else
    ...
    CT.AW=awt:Toggle({Flag="AutoWork",Title="自动模式",Value=false,Callback=function(v)S.AutoWork=v end})
    awt:Divider()
    ...
    CT.TT=ut:Toggle({Flag="TT",Title="透明背景",Value=true,Callback=function(v)pcall(function()if WN then WN:ToggleTransparency(v)end end)end})
    ut:Divider()
    local allT={};pcall(function()allT=WI:GetThemes()end)
    local tn={};for n,_ in pairs(allT)do table.insert(tn,n)end;table.sort(tn)
    CT.TD=ut:Dropdown({Flag="TD",Title="选择主题",Values=tn,Value="Dark",Callback=function(sl)
        if sl and type(sl)=="string"then S.Theme=sl;WI:SetTheme(sl);S.PColor=gtc(sl);updateParticleColors()end
    end})
    ...
    ct:Space()
    ct:Button({Title="💾 保存",Icon="solar:check-circle-bold",Justify="Center",Color=Color3.fromHex("#305dff"),Callback=function()
        if not CM then return end;local c=CM:Config(CF)
        if c and c:Save()then WI:Notify({Title="✅ 已保存",Content="配置 '"..CF.."'",Duration=3,Icon="solar:check-circle-bold"});ACD:Refresh(CM:AllConfigs())end
    end})
    ct:Space()
    ct:Button({Title="📂 加载",Icon="solar:refresh-circle-bold",Justify="Center",Color=Color3.fromHex("#10C550"),Callback=function()
        if not CM then return end;local c=CM:CreateConfig(CF,false)
        if c and c:Load()then WI:Notify({Title="✅ 已加载",Content="配置 '"..CF.."'",Duration=3,Icon="solar:refresh-circle-bold"})end
    end})
    ct:Space()
    ct:Button({Title="🗑️ 删除",Icon="solar:trash-bin-trash-bold",Justify="Center",Color=Color3.fromHex("#ff3040"),Callback=function()
        if not CM then return end;local c=CM:Config(CF)
        if c and c:Delete()then WI:Notify({Title="🗑️ 已删除",Content="配置 '"..CF.."'",Duration=3,Icon="solar:trash-bin-trash-bold"});ACD:Refresh(CM:AllConfigs())end
    end})
    ...
    at:Paragraph({Title="机场安全透视 v15.0",Desc="自动工作系统"})
    ...
    U.InputBegan:Connect(function(i,g)
        ...
        if KB.ESP and KB.ESP~=""and k==KB.ESP then
            S.Enabled=not S.Enabled;if CT.ESP then CT.ESP:Set(S.Enabled)end;refreshESP()
            if S.Enabled then task.spawn(doScan)end
        end
        if KB.BadOnly and KB.BadOnly~=""and k==KB.BadOnly then
            S.BadOnly=not S.BadOnly;if CT.BO then CT.BO:Set(S.BadOnly)end;refreshESP()
        end
    end)
    ...
    task.spawn(function()
        while true do
            task.wait(3)
            pcall(function()doScan();refreshESP();updateStats()end)
        end
    end)
