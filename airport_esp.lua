            ...
            if m and m:IsA("Model")and not isPlayerChar(m)then
                ...
                if hrp then
                    ...
                    if d<=S.WorkRange then
                        local nt=classify(m)
                        if(typeFilter=="Bad"and nt=="Bad")or(typeFilter=="Good"and nt=="Good")then
                            table.insert(results,{model=m,hrp=hrp,dist=d,nt=nt})
                        end
                    end
                end
            end
...
local function autoProcessBad()
    ...
    task.spawn(function()
        ...
        pcall(function()if ATE.ST then ATE.ST:Set("[工作中] 处理坏人: "..npcName)end end)
        ...
        if mode=="Arrest"or mode=="Auto"then
            ...
            if hostile and mode=="Auto"then
                pcall(function()if ATE.ST then ATE.ST:Set("[反击] "..npcName.." 反抗,切换击杀")end end)
                mode="Kill"
            end
        end
            ...
            if back then
                for _,t in ipairs(back:GetChildren())do
                    if t:IsA("Tool")and(t.Name=="MP7A1"or t.Name=="M1911"or t.Name=="Taser")then
                        ...
                        pcall(function()if WI then WI:Notify({Title="💀 击杀",Content="使用"..t.Name.."攻击: "..npcName,Duration=2})end end)
                        break
                    end
                end
            end
...
local function autoReleaseGood()
    if not S.AutoWork then return end
    -- 只在全自动/放行模式下运行
    if S.WorkMode~="Auto"and S.WorkMode~="Release"and S.WorkMode~="Arrest"then return end
    ...
    for _,g in ipairs(goodList)do
        ...
        if not WORK_BUSY[npc]then
            ...
            local npcName="";pcall(function()npcName=npc.Name end)
            task.spawn(function()
                -- 用MarkSearch表示放行（标记为已检查）
                if MarkSearch then
                    pcall(function()MarkSearch:FireServer(npc)end)
                    pcall(function()if ATE.ST then ATE.ST:Set("[放行] "..npcName.." 好人已放行")end end)
                    pcall(function()if WI then WI:Notify({Title="✅ 放行",Content=npcName.." 已放行",Duration=2,Icon="solar:check-circle-bold"})end end)
                end
                task.wait(1)
                WORK_BUSY[npc]=nil
            end)
...
local function startAutoWorkLoop()
    while true do
        ...
        pcall(function()
            if S.AutoWork then
                autoProcessBad()
                if S.WorkMode=="Auto"or S.WorkMode=="Release"then
                    autoReleaseGood()
                end
            elseif ATE.ST then
                ATE.ST:Set("[停止] 自动工作已关闭")
            end
        end)
    end
end
...
local function makeWindow()
    ...
    awt:Paragraph({Title="📋 模式说明",Desc="Arrest=逮捕坏人|Kill=击杀坏人|Release=放行好人|Auto=全自动"})
    ...
    local at=WN:Tab({Title="关于",Icon="solar:info-square-bold"});at:Paragraph({Title="机场安全透视 v15.1",Desc="自动工作+放行"});at:Divider();at:Paragraph({Title="👤 作者",Desc="b站英吉利超入_"});at:Divider();at:Paragraph({Title="💡 使用",Desc=IM and"手机:点击悬浮按钮"or"PC:RightShift打开菜单"})
