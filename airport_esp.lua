...
local function scanProps(m)
    ...
    local sv=rFind(pr,"StatusVariables");if sv then local h=rFind(sv,"Hostile");if h and h:IsA("BoolValue")and h.Value then return true end end
    local rv=rFind(pr,"RandomVariables")
    if rv then
        local c=rFind(rv,"ContrabandReal");if c and c:IsA("BoolValue")and c.Value then return true end
        local f=rFind(rv,"FakePassport");if f and f:IsA("BoolValue")and f.Value then return true end
    end
    return nil
end

local function classify(c)
    if not c then return"Good"end;local nm=c.Name or"";local fp="";pcall(function()fp=c:GetFullName()end)
...
local function classifyLuggage(lug)
    ...
    if pr then local cb=rFind(pr,"Contraband");if cb and cb:IsA("BoolValue")then return cb.Value and"Dangerous"or"Safe"end end
    return"Suspicious"
end
...
local function makeHL(obj,col)
    ...
    hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop;hl.Adornee=obj;hl.Parent=obj;return hl
end
...
local function makeNPC_ESP(c,nt)
    ...
    local hl=makeHL(c,col);local head=c:FindFirstChild("Head")or hrp
...
local function makeLuggage_ESP(lug,lt)
    if not lug or not lug.Parent then return end;if lug.Name~="OpenableLuggage"then return end
    local pp=getLuggagePP(lug);if not pp then return end;local key=pp
    if LG[key]then
    ...
    local hl=makeHL(lug,col);local bb=Instance.new("BillboardGui");bb.Adornee=pp;bb.Size=UDim2.new(0,220,0,56)
...
local function rescanLuggage()
            ...
            if ns then
                for _,fn in ipairs({"LuggageWorkspace","LuggageOpenWorkspace","LuggageEndWorkspace"})do
                    ...
                    if fw then for _,lug in ipairs(fw:GetChildren())do
                        if lug:IsA("Model")and lug.Name=="OpenableLuggage"then local pp=getLuggagePP(lug)
                            if pp and not seenPP[pp]then seenPP[pp]=true;local lt=classifyLuggage(lug);makeLuggage_ESP(lug,lt)end end end end end end end end
    if LC==0 then for _,o in ipairs(W:GetDescendants())do
        if o:IsA("Model")and o.Name=="OpenableLuggage"and o.Parent and o.Parent:IsA("Folder")then
            local pp=getLuggagePP(o);if pp and not seenPP[pp]then seenPP[pp]=true;local lt=classifyLuggage(o);makeLuggage_ESP(o,lt)end end end end
end

local function refreshESP()
    for c,o in pairs(H)do
        if not c or not c.Parent then
            pcall(function()if o.bb then o.bb:Destroy()end;if o.hl then o.hl:Destroy()end end);H[c]=nil
            ...
            if o.lb then local txt=o.tag
                if S.ShowDist and LP.Character then local mp=LP.Character:FindFirstChild("HumanoidRootPart")
...
local function mkParts()
    if not PP then return end;if PC then return end;task.wait(0.5)
    ...
    task.spawn(function()
        ...
        while PR and PC do t=t+0.03
            pcall(function()local curCol=S.PColor
...
local function makeWindow()
    ...
    mt:Divider();CT.DT=mt:Toggle({Flag="Dist",Title="显示距离",Value=false,Callback=function(v)S.ShowDist=v end});CT.HT=mt:Toggle({Flag="Health",Title="显示血量",Value=false,Callback=function(v)S.ShowHP=v end})
    ...
    CT.TT=ut:Toggle({Flag="TT",Title="透明背景(Transparent)",Value=true,Callback=function(v)pcall(function()if WN then pcall(function()WN:ToggleTransparency(v)end)end end)end});ut:Divider()
    ...
    at:Paragraph({Title="机场安全透视 v14.14",Desc="修复WindUI加载失败"})
...
-- 加载WindUI (3次重试 + 超时保护)
local WI=nil;local retryCount=0;local maxRetries=3;local loaded=false
while retryCount<maxRetries and not loaded do
    ...
    if ok and rv then WI=rv;loaded=true
    else retryCount=retryCount+1;if retryCount<maxRetries then task.wait(1)end end end

if loaded then
    pcall(function()WI:SetTheme("Dark")end);S.PColor=gtc("Dark")
    WI:Popup({Title="机场安全透视 v14.14",Icon="solar:info-square-bold",
    ...
    local msg=Instance.new("Message");msg.Text="⚠️ WindUI加载失败(已重试"..maxRetries.."次)";msg.Parent=W
