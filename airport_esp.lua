--[[
    机场安全透视 v15.2
    修复: 自动工作 - 双通道放行/逮捕(RemoteEvent+Tool)
    作者: b站英吉利超入_
]]
...
local function clean()local wc=0;for _,g in ipairs(C:GetChildren())do if g:IsA("ScreenGui")then local n=g.Name;if n:find("WindUI")then wc=wc+1;if wc>1 then pcall(function()g:Destroy()end)end elseif n=="A"or n:find("AirportESP")or n:find("ESP_Particles")then pcall(function()g:Destroy()end)end end end end;clean()
...
-- 调试用：扫描NPC属性并返回文本
local function debugProps(m)
    if not m then return "无Model" end
    local pr=m:FindFirstChild("Properties");if not pr then return "无Properties" end
    local parts={}
    for _,c in ipairs(pr:GetDescendants())do
        if c:IsA("BoolValue")then table.insert(parts,c.Name.."="..tostring(c.Value))
        elseif c:IsA("NumberValue")then table.insert(parts,c.Name.."="..c.Value)
        elseif c:IsA("StringValue")then table.insert(parts,c.Name.."='"..c.Value.."'")
        elseif c:IsA("ObjectValue")and c.Value then table.insert(parts,c.Name.."="..c.Value.Name) end
    end
    if #parts==0 then return "Properties空" end
...
-- 获取背包里的工具
local function getTool(nameList)
...
-- 扫描附近NPC
local function scanNearby(typeFilter)
    local results={}
            ...
            if m and m:IsA("Model")and not isPlayerChar(m)then
                ...
                if hrp then
                    ...
                    if d<=S.WorkRange then
                        local nt=classify(m)
                        local props=debugProps(m)
                        if(typeFilter=="Bad"and nt=="Bad")or(typeFilter=="Good"and nt=="Good")or(typeFilter=="All")then
                            table.insert(results,{model=m,hrp=hrp,hum=o,dist=d,nt=nt,props=props})
                        end
                    end
                end
            end
...
-- 放行(双通道: RemoteEvent + Arrest/Release工具)
local function doRelease(npc,npcName)
    local success=false
    -- 通道1: RemoteEvent
    if MarkSearch then
        local ok,err=pcall(function()MarkSearch:FireServer(npc)end)
        if ok then success=true end
        -- 也试试传Humanoid
        local hum,_=getNPC(npc)
        if hum then
            pcall(function()MarkSearch:FireServer(hum)end)
        end
        task.wait(0.5)
    end
    -- 通道2: 工具内的RemoteEvent
    local arrestTool=getTool({"Arrest"})
    if arrestTool then
        for _,c in ipairs(arrestTool:GetDescendants())do
            if c:IsA("RemoteEvent")or c:IsA("BindableEvent")then
                pcall(function()c:FireServer(npc)end)
                pcall(function()c:FireServer(getNPC(npc))end)
                success=true
            end
        end
    end
    pcall(function()if ATE.ST then ATE.ST:Set("[放行] "..npcName.." ✅")end end)
    pcall(function()if WI then WI:Notify({Title="✅ 已处理",Content=npcName.." 放行",Duration=2,Icon="solar:check-circle-bold"})end end)
    return success
end

-- 逮捕(双通道)
local function doArrest(npc,npcName)
    local success=false
    local hum,_=getNPC(npc)
    -- 通道1: NPCArrest
    if NPCArrest then
        local ok1=pcall(function()NPCArrest:FireServer(npc)end)
        local ok2=pcall(function()NPCArrest:FireServer(hum)end)
        if ok1 or ok2 then success=true end
        task.wait(0.5)
    end
    -- 通道2: MarkArrest
    if MarkArrest then
        pcall(function()MarkArrest:FireServer(npc)end)
        pcall(function()MarkArrest:FireServer(hum)end)
        success=true
        task.wait(0.3)
    end
    -- 通道3: 用Arrest工具
    if not success then
        local arrestTool=getTool({"Arrest"})
        if arrestTool then
            pcall(function()arrestTool.Parent=LP.Character end)
            task.wait(0.2)
            pcall(function()arrestTool:Activate()end)
            success=true
        end
    end
    return success
...
-- 击杀
local function doKill(npc,npcName)
    local hum,_=getNPC(npc)
    if not hum or hum.Health<=0 then return true end
    local weapon=getTool({"MP7A1","M1911","Taser"})
    if weapon then
        pcall(function()weapon.Parent=LP.Character end)
        task.wait(0.2)
        pcall(function()weapon:Activate()end)
        pcall(function()if WI then WI:Notify({Title="💀 击杀",Content="使用"..weapon.Name.."攻击: "..npcName,Duration=2})end end)
    else
        -- 没有武器,尝试用Taser
        local taser=getTool({"Taser"})
        if taser then
            pcall(function()taser.Parent=LP.Character end)
            task.wait(0.2)
...
-- 主处理
local function autoProcessBad()
    if not S.AutoWork then return end
    local badList=scanNearby("Bad")
    if #badList==0 then
        -- 显示调试信息: 扫到了什么
        local allList=scanNearby("All")
        if #allList>0 then
            local info="附近有"..#allList.."个NPC"
            pcall(function()if ATE.ST then ATE.ST:Set("[信息] "..info)end end)
            print("[自动工作] 附近NPC:")
            for _,n in ipairs(allList)do
                print("  "..n.model.Name.." 分类="..n.nt.." 距离="..math.floor(n.dist).."m 属性="..n.props)
            end
        else
            pcall(function()if ATE.ST then ATE.ST:Set("[空闲] 范围内无可处理NPC")end end)
        end
        return
    end
    local npc=badList[1].model
    local npcName="";pcall(function()npcName=npc.Name end)
    if WORK_BUSY[npc]then return end
    WORK_BUSY[npc]=true
    local mode=S.WorkMode
    task.spawn(function()
        pcall(function()if ATE.ST then ATE.ST:Set("[工作中] "..npcName.." ("..mode..")")end end)
        if mode=="Arrest"or mode=="Auto"then
            local arrested=doArrest(npc,npcName)
            if arrested then
                local hostile=scanProps(npc)
                if hostile and mode=="Auto"then
                    pcall(function()if ATE.ST then ATE.ST:Set("[反击] "..npcName.." 转为击杀")end end)
                    doKill(npc,npcName)
                end
            end
        end
        ...
        task.wait(2)
        WORK_BUSY[npc]=nil
        pcall(function()if ATE.ST then ATE.ST:Set("[完成] "..npcName.." 已处理")end end)
    end)
end

local function autoReleaseGood()
    if not S.AutoWork then return end
    if S.WorkMode~="Auto"and S.WorkMode~="Release"then return end
    local goodList=scanNearby("Good")
    for _,g in ipairs(goodList)do
        local npc=g.model
        if not WORK_BUSY[npc]then
            WORK_BUSY[npc]=true
            local npcName="";pcall(function()npcName=npc.Name end)
            task.spawn(function()
                doRelease(npc,npcName)
                task.wait(0.5)
                WORK_BUSY[npc]=nil
            end)
        end
...
local function startAutoWorkLoop()
    while true do
        ...
        pcall(function()
            if S.AutoWork then
                autoProcessBad()
                if S.WorkMode=="Auto"or S.WorkMode=="Release"then autoReleaseGood()end
...
local function makeWindow()
    ...
    awt:Paragraph({Title="📋 模式说明",Desc="Arrest=逮捕|Kill=击杀|Release=放行|Auto=全自动\n自动工作不依赖透视开关,独立运行"})
    ...
    local at=WN:Tab({Title="关于",Icon="solar:info-square-bold"});at:Paragraph({Title="机场安全透视 v15.2",Desc="双通道自动工作"});at:Divider();at:Paragraph({Title="👤 作者",Desc="b站英吉利超入_"});at:Divider();at:Paragraph({Title="💡 使用",Desc=IM and"手机:点击悬浮按钮"or"PC:RightShift打开菜单"})
