--[[
    ...
    v12.7 修复4个Bug:
    1. NPC检测不到 → 双扫描法(Humanoid+Head全覆盖)
    2. 无法判断 → 默认坏人
    3. 悬浮按钮 → 立即创建
    4. 透明背景 → 放在正确位置
]]
...
local function me(c,nt)
    ...
    if S.DebugMode then
        local at="";pcall(function()for _,a in ipairs(c:GetAttributes())do at=at..a.."="..tostring(c:GetAttribute(a)).." "end end)
        local h2=c:FindFirstChildOfClass("Humanoid");if h2 then pcall(function()for _,a in ipairs(h2:GetAttributes())do at=at..a.."="..tostring(h2:GetAttribute(a)).." "end end)end
...
local function sc()
    ...
    pcall(function()
        for _,o in ipairs(WS:GetDescendants())do
            if o:IsA("Humanoid")then local c=o.Parent;if c and not TR[c]and not isP(c)and not viewed[c]then viewed[c]=true;local nt=classify(c);if nt=="Good"then ST.G=ST.G+1 else ST.B=ST.B+1 end;if not EO[c]then me(c,nt)end end end
        end
        for _,o in ipairs(WS:GetDescendants())do
            if o:IsA("BasePart")and o.Name=="Head"then local c=o.Parent;if c and c:IsA("Model")and not TR[c]and not isP(c)and not viewed[c]then viewed[c]=true;local nt=classify(c);if nt=="Good"then ST.G=ST.G+1 else ST.B=ST.B+1 end;if not EO[c]then me(c,nt)end end end
        end
    end)
...
if ok and rv then
    ...
    function cw()
        ...
        ut:Divider();ut:Paragraph({Title="🔄 透明控制"});CT.TT=ut:Toggle({Flag="TransToggle",Title="透明背景",Value=true,Callback=function(v)if v then pcall(function()WI.TransparencyValue=0.22 end)else pcall(function()WI.TransparencyValue=0 end)end end})
