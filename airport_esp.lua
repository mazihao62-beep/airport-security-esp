...
local function rf(i,n)
    ...
    for _,c in ipairs(i:GetChildren())do if c:IsA("Configuration")or c:IsA("Folder")then local r=rf(c,n);if r then return r end end end
    return nil
end
...
local function glp(lug)
    ...
    if not pp then for _,c in ipairs(lug:GetDescendants())do if c:IsA("BasePart")and(c.Name=="ColorFront"or c.Name=="ColorBack"or c.Name=="HumanoidRootPart")then pp=c;break end end end
...
local function doScan()
    upS();local seen={}
    for _,o in ipairs(W:GetDescendants())do if o:IsA("Humanoid")then local c=o.Parent
        if c and c:IsA("Model")and not seen[c]then seen[c]=true;if not PCSet[c]then mNESP(c,cl(c))end end end end
end

local function rLBL()
    local seenPP={};local wsf=W:FindFirstChild("WorkspaceScriptable")
    if wsf then local sto=wsf:FindFirstChild("Storage")
        if sto then local ns=sto:FindFirstChild("NormalStorage")
            if ns then for _,fn in ipairs({"LuggageWorkspace","LuggageOpenWorkspace","LuggageEndWorkspace"})do local fw=ns:FindFirstChild(fn)
                if fw then for _,lug in ipairs(fw:GetChildren())do if lug:IsA("Model")and lug.Name=="OpenableLuggage"then local pp=glp(lug)
                    if pp then local key=pp;seenPP[key]=true;local lt=cll(lug)
                        if LG[key]then if LG[key].nt~=lt then LG[key].nt=lt;LG[key].tag=lt=="Dangerous"and"💣 危险行李"or(lt=="Safe"and"🧳 安全行李"or"❓ 可疑行李")
                            if LG[key].lb then local col=lt=="Dangerous"and Color3.fromRGB(255,40,40)or(lt=="Safe"and Color3.fromRGB(0,255,80)or Color3.fromRGB(255,180,40))
                                LG[key].lb.TextColor3=col;LG[key].lb.Text=LG[key].tag end;if LG[key].hl then LG[key].hl.FillColor=col end end
...
local function iLS()
    ...
    LG={};LC=0;LDC=0;LSC=0;rLBL()
end

local function rESP()
    for c,o in pairs(H)do if not c or not c.Parent then pcall(function()if o.bb then o.bb:Destroy()end;if o.hl then o.hl:Destroy()end end);H[c]=nil
            ...
            if o.lb then local txt=o.tag
                ...
                if S.ShowHP then local h2=c:FindFirstChildOfClass("Humanoid")
                    if h2 then txt=txt.." | HP:"..math.floor(h2.Health+0.5).."/"..math.floor(h2.MaxHealth+0.5)end end;o.lb.Text=txt end end end
    for _,o in pairs(LG)do if o.bb then o.bb.Enabled=S.Luggage end;if o.hl then o.hl.Enabled=S.Luggage end end
end

local function uS()
    GC,BC,SC,LC,LDC,LSC=0,0,0,0,0,0
...
local function sNN()
    ...
    for _,o in ipairs(W:GetDescendants())do if o:IsA("Humanoid")then local m=o.Parent
...
local function eF(wn)
    ...
    if LP.Character then for _,t in ipairs(LP.Character:GetChildren())do if t:IsA("Tool")then pcall(function()t.Parent=back end)end end end;task.wait(0.1)
    local function fw()if back then for _,t in ipairs(back:GetChildren())do if t:IsA("Tool")and t.Name==wn then return t end end end
        if LP.Character then for _,t in ipairs(LP.Character:GetChildren())do if t:IsA("Tool")and t.Name==wn then return t end end end;return nil end
    local t=fw();if not t then return false end
    pcall(function()t.Parent=LP.Character end);task.wait(0.15);pcall(function()t:Activate()end);task.wait(0.1)
    for i=1,3 do pcall(function()VIM:SendMouseButtonEvent(0,0,0,true,game,1)end);task.wait(0.05)
        pcall(function()VIM:SendMouseButtonEvent(0,0,0,false,game,1)end);task.wait(0.03)end;return true
end
...
local function dA(n)
    ...
    if LP.Character and back then for _,t in ipairs(LP.Character:GetChildren())do if t:IsA("Tool")and t.Name=="Arrest"then pcall(function()t.Parent=back end)end end end
    task.wait(0.2);tJ(n)
end
...
local function dK(n)
    ...
    for i=1,8 do task.wait(0.08);pcall(function()VIM:SendMouseButtonEvent(0,0,0,true,game,1)end);task.wait(0.04)
        pcall(function()VIM:SendMouseButtonEvent(0,0,0,false,game,1)end)end
end
...
local function aP()
    ...
    for _,n in ipairs(list)do if WB[n.model]then continue end;local b=n.isBad
        if((mode=="Arrest"or mode=="Kill")and not b)then continue end;if(mode=="Release"and b)then continue end
        WB[n.model]=true;local nm=n.model.Name
        task.spawn(function()
            if b then if mode=="Kill"then pcall(function()if ATE.ST then ATE.ST:Set("[击杀] "..nm)end end);dK(n.model)
            ...
            task.wait(3);WB[n.model]=nil end)end
end
...
local function mkP()
    ...
    local col=S.PColor
    for i=1,50 do local d=Instance.new("Frame");local sz=math.random(5,10);d.Size=UDim2.new(0,sz,0,sz);local sx=0.2+math.random()*0.6;local sy=0.2+math.random()*0.6
        ...
        Instance.new("UICorner",d).CornerRadius=UDim.new(0,10);local a=math.random()*6.28;local sp=0.0008+math.random()*0.002
        table.insert(PS,{F=d,Sx=sx,Sy=sy,Vx=math.cos(a)*sp,Vy=math.sin(a)*sp,Ph=math.random()*6.28,Sz=sz})end;PR=true
    task.spawn(function()local t=0;while PR and PC do t=t+0.03;pcall(function()local cc=S.PColor
        for _,p in ipairs(PS)do if p.F and p.F.Parent then local sx=math.max(0.05,math.min(0.95,p.Sx+p.Vx));local sy=math.max(0.05,math.min(0.95,p.Sy+p.Vy))
            if sx>=0.95 or sx<=0.05 then p.Vx=-p.Vx end;if sy>=0.95 or sy<=0.05 then p.Vy=-p.Vy end;p.Sx=sx;p.Sy=sy;p.F.Position=UDim2.new(sx,0,sy,0)
            if cc~=p.F.BackgroundColor3 then p.F.BackgroundColor3=cc end;p.F.BackgroundTransparency=0.3+math.sin(t*0.8+p.Ph)*0.4
            local bs=math.max(2,p.Sz+math.sin(t+p.Ph)*1.5);p.F.Size=UDim2.new(0,bs,0,bs)end end end);task.wait(0.03)end end)
end
